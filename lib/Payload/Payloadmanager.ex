defmodule Payload.Manager do
  ...
  def handle_call({:meta}, _from, state) do
    stats = Payload.Variant.stats(state.data, state.arena, state.time)
    meta = %Payload.Meta{
      id: state.game_id,
  ...
  def handle_call({:control, control_params}, _from, state) do
    {result, new_data, new_commands, notifications} =
      Payload.Variant.control(state.data, state.arena, state.time, control_params)
  ...
  #  update_event = %Payload.Events.ArenaUpdated{time: time, arena: arena}

#    {new_data, new_commands} =
      #[update_event | events]
      #|> Enum.reduce({state.data, state.commands}, fn event, {data, commands} ->
        #{ndata, command, notifications} = Payload.Variant.event(data, event)
  ...
  defp state_from_handoff(base_state, handoff_state) do
    ...
    GenServer.cast(
      Payload.updater_process_id(handoff_state.game_id),
      {:up, self(), handoff_state.arena, opts}
    )

   # notification = %Payload.Notifications.Moved{
  ...
  defp do_up(base_state, data) do
    ...
    arena = Payload.Variant.init_arena(data, time)
    #start_event = %Payload.Events.ArenaUpdated{time: time, arena: arena}
    #GenServer.cast(Payload.updater_process_id(base_state.game_id), {:up, self(), arena, opts})
    #{data, commands, _notifications} = Payload.Variant.event(data, start_event)
...
#        opts: opts,
 #       data: data,
  ...
  defp do_handoff(state) do
    GenServer.cast(Payload.updater_process_id(state.game_id), {:down})
  ...
  defp do_down(state) do
    ...
    notification_data = Payload.Variant.stop(state.data, state.arena, state.time)

#    notification = %Payload.Notifications.Ended{
#      id: state.game_id,
#      time: state.time,
